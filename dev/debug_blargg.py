...
parser.add_argument("rom", type=str, help="path to a rom")
...
def print_cartridge_info(cart):
    ...
    print("Format: iNES")
    print(f"Nb PRG-ROM banks: {cart.nb_rom_banks}")
    print(f"Nb CHR-ROM/VROM banks: {cart.nb_vrom_banks}")
...
for i in range(args.nb_runs):
    ...
    # The Blarg test suites initializes memory 0x6001-0x6003 to let the test running the emulator
    # that the result at 0x6000 can be read to interpret the result of the tests.
    # So loop until we reach that milestone.
    while (
